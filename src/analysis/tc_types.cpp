...
shared_ptr<analysis::Type> analysis::tc_type(shared_ptr<ast::Type> type, shared_ptr<analysis::Scope> env) {
  switch (type->kind) {
  case SYMBOL_TYPE: {
  ...
  case POINTER_TYPE:
    return MK_PTR(tc_type(static_cast<ast::PointerType *>(type.get())->type, env));
